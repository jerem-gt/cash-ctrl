  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-3 flex-wrap items-center">
        ...
        <Select className="flex-1 min-w-32.5 max-w-50" value={String(filters.category_id ?? '')} onChange={e => setFilter({ category_id: e.target.value ? Number.parseInt(e.target.value) : undefined })}>
          <option value="">Toutes catégories</option>
          {categories.map(c => <option key={c.id} value={String(c.id)}>{c.name}</option>)}
        </Select>
        <Select className="flex-1 min-w-32.5 max-w-50" value={filters.type ?? ''} onChange={e => setFilter({ type: (e.target.value || undefined) as 'income' | 'expense' | undefined })}>
      ...
      {(totalPages > 1 || total > 10) && (
        <Pagination page={page} totalPages={totalPages} total={total} limit={limit} onChange={setPage} onLimitChange={handleLimitChange} />
      )}
