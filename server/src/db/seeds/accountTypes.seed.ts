...
export function seedAccountTypes(db: Database, userId: number) {
  const insert = db.prepare(
    'INSERT OR IGNORE INTO account_types (user_id, name, is_investment, is_loan) VALUES (?, ?, ?, ?)',
  );
  const update = db.prepare(
    'UPDATE account_types SET is_investment = ?, is_loan = ? WHERE name = ? AND user_id = ?',
  );

  db.transaction(() => {
    for (const { name, is_investment, is_loan } of DEFAULT_ACCOUNT_TYPES) {
      insert.run(userId, name, is_investment, is_loan);
      update.run(is_investment, is_loan, name, userId);
    }
  })();
}
