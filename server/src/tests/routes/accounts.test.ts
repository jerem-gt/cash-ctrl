...
describe('/api/accounts', () => {
  ...
  it('POST / creates an account', async () => {
    const res = await ctx.agent.post('/api/accounts').send({ name: 'Courant', bank: 'BNP', type: 'Courant', initial_balance: 500 });
  ...
  it('POST / returns 400 on missing name', async () => {
    const res = await ctx.agent.post('/api/accounts').send({ bank: 'BNP', type: 'Courant' });
    expect(res.status).toBe(400);
  });

  it('PUT /:id updates an account', async () => {
    const create = await ctx.agent.post('/api/accounts').send({ name: 'ToUpdate', bank: 'X', type: 'Courant' });
    const id = create.body.id;
    const res = await ctx.agent.put(`/api/accounts/${id}`).send({ name: 'Updated', bank: 'Y', type: 'Épargne', initial_balance: 0 });
    expect(res.status).toBe(200);
    expect(res.body.name).toBe('Updated');
    expect(res.body.bank).toBe('Y');
  });

  it('PUT /:id returns 404 for unknown account', async () => {
    const res = await ctx.agent.put('/api/accounts/99999').send({ name: 'x', bank: 'x', type: 'x' });
    expect(res.status).toBe(404);
  });

  it('DELETE /:id deletes an account', async () => {
    const create = await ctx.agent.post('/api/accounts').send({ name: 'ToDelete', bank: 'X', type: 'Courant' });
    const id = create.body.id;
