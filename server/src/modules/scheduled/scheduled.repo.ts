...
export function createScheduledRepo(db: Database) {
  return {
    getByUserId(userId: number): ScheduledTransaction[] {
      return db.prepare<[number], ScheduledTransaction>(`
        SELECT s.id,
               s.user_id,
               s.account_id,
               s.to_account_id,
    ...
    getById(id: number, userId: number): ScheduledTransaction | undefined {
      if (!db.prepare('SELECT id FROM scheduled_transactions WHERE id = ? AND user_id = ?').get(id, userId)) return undefined;
      return db.prepare<[number], ScheduledTransaction>(SCHED_WITH_DETAILS).get(id) ?? undefined;
    },

    exists(id: number, userId: number): boolean {
      return !!db.prepare('SELECT id FROM scheduled_transactions WHERE id = ? AND user_id = ?').get(id, userId);
    },

    getTransferPmId(): number | undefined {
      return db.prepare<[], { id: number }>(`SELECT id
                                             FROM payment_methods
                                             WHERE name = 'Transfert'`).get()?.id;
    },
    ...
    create(userId: number, data: CreateScheduledInput) {
      return db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
          userId, data.account_id, data.to_account_id, data.type, data.amount, data.description,
          data.category_id, data.payment_method_id, data.notes,
          data.recurrence_unit, data.recurrence_interval, data.recurrence_day, data.recurrence_month,
          data.weekend_handling, data.start_date, data.end_date, data.active ? 1 : 0,
      );
    },
    ...
    update(userId: number, id: number, data: CreateScheduledInput) {
      ...
      db.transaction(() => {
        db.prepare('DELETE FROM transactions WHERE scheduled_id = ? AND user_id = ? AND date > ? AND validated = 0').run(id, userId, today);
        db.prepare(`
          ...
          WHERE id = ?
            AND user_id = ?
        `).run(
            data.account_id, data.to_account_id, data.type, data.amount, data.description,
            data.category_id, data.payment_method_id, data.notes,
            data.recurrence_unit, data.recurrence_interval,
            data.recurrence_day, data.recurrence_month, data.weekend_handling,
            data.start_date, data.end_date, data.active ? 1 : 0,
            id, userId,
        );
      })();
    },
